    ...
    <div className="p-8">
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-1">Geographic regions and property zones</p>
        </div>
          ...
          return (
            <div key={region} className="bg-gray-800 rounded-xl border border-gray-700 p-6">
              <div className="flex items-center gap-2 mb-4">
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${regionColors[region]}`}>{region}</span>
                <span className="text-gray-400 text-sm">{territories.length} territories</span>
              </div>
                ...
                {territories.map(t => (
                  <div key={t.id} className="flex items-center justify-between bg-gray-700/50 rounded-lg p-3">
                    <div>
                      ...
                      <p className="text-gray-400 text-xs">{t.country} Â· {t.hotels} hotels Â· {t.manager}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-green-400 text-sm font-medium">{t.revenue}</p>
      ...
      <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
        <div className="p-6 border-b border-gray-700">
          <h2 className="text-white font-semibold">All Territories</h2>
        </div>
              ...
              {mockTerritories.map((t) => (
                <tr key={t.id} className="hover:bg-gray-700/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs ${regionColors[t.region]}`}>{t.region}</span>
                  </td>
                  <td className="px-6 py-4 text-gray-300 text-sm">{t.country}</td>
                  <td className="px-6 py-4 text-gray-300 text-sm">{t.hotels}</td>
                  <td className="px-6 py-4 text-gray-300 text-sm">{t.activeBookings}</td>
                  <td className="px-6 py-4 text-green-400 text-sm font-medium">{t.revenue}</td>
                  <td className="px-6 py-4 text-gray-300 text-sm">{t.manager}</td>
                  <td className="px-6 py-4">
